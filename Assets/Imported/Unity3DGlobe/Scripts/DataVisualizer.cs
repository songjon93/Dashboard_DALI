...
public class DataVisualizer : MonoBehaviour {
    ...
    List<Color> meshColors)
    {
        ...
        Debug.Log(p.transform.localScale);
        p.transform.localScale = new Vector3(3, 3, Mathf.Max(0.001f, value * ValueScaleMultiplier));
    ...
    private void MarkSelected(GameObject p, Vector3[] verts, int[] indices, float lng, float lat, float value, List<Vector3> meshVertices,
    List<int> meshIndices, List<Color> meshColors)
    {
        Color valueColor = Colors.Evaluate(1);
        Vector3 pos;
        pos.x = 0.5f * Mathf.Cos((lng) * Mathf.Deg2Rad) * Mathf.Cos(lat * Mathf.Deg2Rad);
        pos.y = 0.5f * Mathf.Sin(lat * Mathf.Deg2Rad);
        pos.z = 0.5f * Mathf.Sin((lng) * Mathf.Deg2Rad) * Mathf.Cos(lat * Mathf.Deg2Rad);
        ...
        p.transform.position = pos;
        Debug.Log(p.transform.localScale);
        p.transform.localScale = new Vector3(5, 5, Mathf.Max(0.001f, value * ValueScaleMultiplier));
        p.transform.LookAt(pos * 2);
    ...
    private Mesh CreateObject(List<Vector3> meshertices, List<int> meshindecies, List<Color> meshColors, GameObject seriesObj)
    {
        Mesh mesh = new Mesh();
